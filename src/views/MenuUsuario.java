...
public class MenuUsuario extends JFrame {
	...
	public MenuUsuario() {
		...
		btnSalir.setBounds(27, 490, 200, 34);
		...
		btnSalir.addMouseListener(new MouseAdapter() {
            ...
            @Override
            public void mouseEntered(MouseEvent e) {
            	Color colorRGB = new Color(27, 162, 245);
            	btnSalir.setForeground(SystemColor.blue);
            	btnSalir.setBackground(colorRGB); // Cambia el color de fondo a amarillo cuando el mouse entra
            	btnSalir.setOpaque(true); // Asegura que el fondo sea opaco para que se muestre el color
            	
            }
            @Override
            public void mouseExited(MouseEvent e) {
            	btnSalir.setBackground(null); // Restaura el color de fondo a null para que use el color predeterminado cuando el mouse sale
            	btnSalir.setOpaque(false); // Desactiva la opacidad para que vuelva a ser transparente
            	btnSalir.setForeground(Color.WHITE);
            }
		});
		...
		btnexit.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				int respuesta = JOptionPane.showConfirmDialog(null, "¿Desea salir de la aplicación?", "Confirmación", JOptionPane.YES_NO_OPTION);
	            if (respuesta == JOptionPane.YES_OPTION) {
	                System.exit(0); // Cierra la aplicación si el usuario selecciona "Sí"
	            }
			}
			@Override
			public void mouseEntered(MouseEvent e) {
    ...
    private void salirMenuPricipal() {
    	int respuesta = JOptionPane.showConfirmDialog(null, "¿Desea salir al menú principal?", "Confirmación", JOptionPane.YES_NO_OPTION);
        if (respuesta == JOptionPane.YES_OPTION) {
        	// Cierra actual
            dispose();
